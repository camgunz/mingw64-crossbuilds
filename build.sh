...
function build_toolchain_file () {
    ...
    echo "SET(CMAKE_SYSTEM_NAME $CMAKE_SYSTEM_NAME)" > $1
...
do
    ...
    if [ -f config.sub ]
    then
        echo "Downloading updated config.sub"
        wget "https://gcc.gnu.org/svn/gcc/trunk/config.sub" && \
            rm -f config.sub && \
            mv config.sub.1 config.sub
    fi
